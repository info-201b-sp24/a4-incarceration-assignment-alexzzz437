...
incarceration_map_data <- map_data("state")

incarceration_rates <- prison_jail_rates %>%
  filter(!is.na(total_jail_pop_rate)) %>%
  group_by(state) %>%
  summarize(incarceration_rate = mean(total_jail_pop_rate, na.rm = TRUE))

# Add a column with state names in lowercase to match the map data
incarceration_rates$state <- tolower(incarceration_rates$state)

# Merge the datasets
merged_data <- incarceration_map_data %>%
  left_join(incarceration_rates, by = c("region" = "state"))
